...
class MissingIndicatorPure():
    ...
    def _get_missing_features_info(self, X):
        ...
        if self.features == 'missing-only':
            n_missing = apply_axis_2d(
                imputer_mask, sum, axis=0)
...
class SimpleImputerPure():
    ...
    def _concatenate_indicator(self, X_imputed, X_indicator):
        ...
        return [
            X_imputed[index] + X_indicator[index]
            for index in range(len(X_imputed))
            ]
    ...
    def transform(self, X):
        ...
        if self.strategy == "constant":
            ...
            to_remove = [
                index for index in range(len(self.statistics_)) 
                if isnan(self.statistics_[index])
                ]
            if len(to_remove) > 0:
                X = [
                    [a[i] for i in range(len(a)) if i not in to_remove] 
                    for a in X
                    ]
                valid_statistics = [
                    self.statistics_[i] for i in range(len(self.statistics_)) 
        ...
        func = (
            lambda a,i: a[i] if not _to_impute(a[i], self.missing_values) 
